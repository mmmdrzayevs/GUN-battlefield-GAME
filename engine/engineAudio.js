...
let audioVolume = .5;        // master volume for sound, music and speech
let sfxVolume = 1;           // additional channel volume multiplier for playSound() effects, set by the settings system
let musicVolume = 1;         // additional channel volume multiplier for playMusic() tracks, set by the settings system
...
function playSound(zzfxSound, pos, range=defaultSoundRange, volumeScale=1)
{
    ...
    // scale volume (distance falloff plus the SFX channel volume from settings)
    const scale = volumeScale * sfxVolume * percent(lengthSquared**.5, range, maxRange);
...
function playMusic(zzfxmMusic, loop=1) 
{
    ...
    // scale the rendered music buffers by the music channel volume (and master volume) before
    // handing them to zzfxP, so the music/master sliders in settings affect music playback too
    const musicBuffers = zzfxM(...zzfxmMusic);
    const scale = musicVolume * audioVolume;
    if (scale != 1)
        for (const channel of musicBuffers)
            for (let i = channel.length; i--;)
                channel[i] *= scale;

    const source = zzfxP(...musicBuffers);
...
///////////////////////////////////////////////////////////////////////////////
// ZzFXMicro - Zuper Zmall Zound Zynth - v1.1.8 by aashuu
...
///////////////////////////////////////////////////////////////////////////////
// ZzFX Music Renderer v2.0.3 by Keith Clark and aashuu
